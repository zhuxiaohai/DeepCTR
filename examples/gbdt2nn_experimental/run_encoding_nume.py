...
def online_encoding():
    ec.fit_transform(pd.read_csv(train_csv_path), out_dir + '/train')
    df = pd.read_csv(test_csv_path)
    df = df[df['set'] == '2test']
    ec.transform(df, out_dir + '/test')

    data_type = ['2test', '3oot', '4oot', '5oot']
    for i in range(len(data_type) - 1):
        df = pd.read_csv(test_csv_path)
        df['selffill_marital_status'] = df['selffill_marital_status'].fillna('<LESS>')
        df['is_male'] = df['is_male'].fillna('<LESS>')
        ec.refit_transform(df[df['set'] == data_type[i]], out_dir + '/{}_train'.format(data_type[i]))
        ec.transform(df[df['set'] == data_type[i + 1]], out_dir + '/{}_test'.format(data_type[i + 1]))
...
if online:
    ...
    for data_type in ['2test', '3oot', '4oot', '5oot']:
        ...
        np.save(out_dir + '/test_{}_features.npy'.format(data_type), vld_x)
        np.save(out_dir + '/test_{}_labels.npy'.format(data_type), vld_y)
