...
class MultiTaskModelBase(keras.Model):
    def compile(self, optimizer, loss, metrics, loss_weights=None,
        ...
        if not isinstance(metrics, dict):
            raise ValueError("metrics_logger must be a dict")
        if loss_weights is None:
            self.loss_weights = {task_name: 1.0 for task_name in loss.keys()}
        else:
            if not isinstance(loss_weights, dict):
                raise ValueError("loss_weights must be a dict")
            self.loss_weights = loss_weights
        ...
        with tf.name_scope('metrics_logger'):
            ...
            for task_name, metric in metrics.items():
                with tf.name_scope(task_name + "_" + metric.name):
                    self.metrics_logger[task_name] = metric
            ...
            for task_name in self.loss_fns.keys():
                if self.loss_fns[task_name] is not None:
                    self.log_vars[task_name] = self.add_weight(name=task_name + '_log_var', shape=(1,),
                                                               initializer=Constant(0.), trainable=True)
                    with tf.name_scope('metrics_logger'):
                        with tf.name_scope(task_name + "_exp_log_var"):
                            self.log_vars_logger[task_name] = keras.metrics.Mean(name=task_name + "_exp_log_var")
    ...
    def compute_loss_gradnorm(self, data, training):
        if len(data) == 3:
            x, y, sample_weight = data
        else:
            x, y = data
        ...
        self.total_loss_logger.update_state(total_loss)
        return total_loss, grad_norms_loss

    def _compiled_loss(self, y, y_pred, sample_weight, regularization_losses=None):
        ...
        if regularization_losses is not None:
            total_loss += sum(regularization_losses)
        self.total_loss_logger.update_state(total_loss)
        return total_loss
    ...
    def train_step(self, data):
        if len(data) == 3:
            ...
            x, y = data
            sample_weight = {task_name: tf.ones_like(tf.reduce_sum(y[task_name], axis=-1)) for task_name in y.keys()}
        if self.gradnorm_config is None:
            with tf.GradientTape() as tape:
                y_pred = self(x, training=True)
                loss = self._compiled_loss(y, y_pred, sample_weight, regularization_losses=self.losses)
            grads = tape.gradient(loss, self.trainable_weights)
            self.optimizer.apply_gradients(zip(grads, self.trainable_weights))
        # TODO: merge "compute_loss_gradnorm" to "compiled_loss"
        else:
            with tf.GradientTape(persistent=True) as tape:
                loss, grad_norms_loss = self.compute_loss_gradnorm(data, training=True)
            ...
            grads = tape.gradient(loss, original_weights)
            dynamic_weights = list(self.dynamic_weights.values())
            dynamic_weights_grad = tape.gradient(grad_norms_loss, dynamic_weights)
            self.optimizer.apply_gradients(zip(grads + dynamic_weights_grad, original_weights + dynamic_weights))
        return {m.name: m.result() for m in self.metrics}
    ...
    def test_step(self, data):
        ...
        if self.gradnorm_config is None:
            y_pred = self(x, training=False)
            self._compiled_loss(y, y_pred, sample_weight, regularization_losses=self.losses)
        else:
            _, _ = self.compute_loss_gradnorm(data, training=False)
