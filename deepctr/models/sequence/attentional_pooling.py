...
from tensorflow.python.keras.layers import Dense, Input, Concatenate, Flatten
...
class SelfAttentionalPoolingLayer(Layer):
    """The Attentional sequence pooling operation used in Rnn like structures.
      ...
      Arguments
        ...
        - **return_score**: bool. If return attention score for every time step
        - **attention_factor**: int. Attentional weight unit number
        - **l2_reg_w**: float. attentional l2 regularization
        - **seed**: int. Seed used in weight initialization
        - **supports_masking**:If True,the input need to support masking.
            Note that the function compute_mask is only available to take effect when support masking is True.
            If support masking = True, the fed in mask will be transferred to output,
              and if at the same time compute_mask is defined the newly defined mask will be transferred to output.
            If support masking = False, even if there is fed in mask and it is used, no mask will be transferred out.
      References
...
class ModifiedDynamicGRU(Layer):
    """
    Input, concated_embeds_value (None * time_step * feature_num)
           sequence_len (None, 1)
    Output: None * time_step * embedding if return_sequence is True, embeddings of all steps
            None * 1 * embedding if return_sequence is False, the embedding for the last step

    num_units: embedding_size
    return_sequence: if return embeddings for all steps or embedding for the final step
    ...
    def build(self, input_shape):
        ...
        self.rnn_layer = tf.keras.layers.GRU(self.num_units,
                                             return_state=True,
    ...
    def call(self, input_list, mask=None, training=None):
        ...
        rnn_output, hidden_state = self.rnn_layer(rnn_input, mask=rnn_masks, training=training)
...
def get_sequence_pooling(sequence_input, sequence_length,
    ...
    """
    sequence_input: (None, L, f)
    sequence_length: (None, 1)
    embedding_size: int
    gru_type: str, GRU or GRU_AVG,
              for GRU return the embedding of the final step
              for GRU_AVG return weighted sum of embeddings of all time steps
    att_weight_normalization: bool, if normalize the weights of embeddings
    attention_factor: attentional weight unit number
    l2_reg_w: attentional l2 regularization

    Returns: sequence embeddings (None, 1, embedding_size)
    """
    if gru_type == 'GRU':
...
def AttentionalPooling(constant_feature_columns, sequence_feature_columns, sequence_sparse_indicator,
    ...
    :param sequence_sparse_indicator: list, to indicate sequence sparse field
    ...
    :param gru_type: str, GRU or AVG_GRU
    :param gru_hidden_size: int
    :param gru_attention_w_dim: int, attentional weight unit number
    :param gru_attention_w_l2: float, attentional l2 regularization
    ...
    varlen_sparse_feature_columns = list(
        filter(lambda x: isinstance(x, VarLenSparseFeat), sequence_feature_columns) if sequence_feature_columns else [])
    ...
    history_fc_names = list(map(lambda x: "hist_" + x, sequence_sparse_indicator))
    history_length = features['seq_length']

    # embedding dict for all feature types
    embedding_dict = create_embedding_matrix(constant_feature_columns + sequence_feature_columns,
                                             l2_reg_embedding, seed, prefix="", seq_mask_zero=False)

    # sequence embedding
    if len(varlen_dense_feature_columns) > 0:
    ...
    if (len(varlen_dense_feature_columns) > 0) and (len(varlen_sparse_feature_columns) > 0):
        history_input = Concatenate()([history_emb, history_dense])
    elif len(varlen_dense_feature_columns) > 0:
        history_input = history_dense
    else:
        history_input = history_emb
    hist = get_sequence_pooling(history_input, history_length,
    ...
    if len(constant_sparse_feature_columns) > 0:
        dnn_input_emb_list = embedding_lookup(embedding_dict, features, constant_sparse_feature_columns,
                                              mask_feat_list=sequence_sparse_indicator, to_list=True)
