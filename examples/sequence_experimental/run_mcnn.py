...
if __name__ == '__main__':
    ...
    model = MCNNModel(constant_feature_columns, behavior_feature_columns,
                      sequence_dense_normalizer=sequence_normalizer,
                      constant_dense_normalizer=constant_normalizer,
                      loop_num_list=[2, 2, 2],
                      filters_list=[64, 128, 256],
                      kernel_sizes_list=[[3, 6, 12], [3, 6, 12], [3, 6, 12]],
                      pool_size_list=[3, 3, None],
                      strides_list=[2, 2, None],
                      dnn_hidden_units=[4, 4], dnn_dropout=0.6)
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001, clipvalue=1.0)
    model.compile(optimizer, "binary_crossentropy", run_eagerly=True)
